...
- JWT issuer: tokens are minted with `iss https://mvplean.com` until the relay accepts both issuers (ZA-291), because the relay checks `iss` with an exact match. Setting `ZEROAGENT_JWT_ISSUER=https://zeroagenthq.com` on the project then flips minting; `api/_lib/jwt.js` accepts both values throughout (see `api/_lib/issuer.js`). Do not hardcode the new issuer before that.
